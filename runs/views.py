...
def add_run(request):
    if request.method == "POST":
        ...
        Run.objects.create(date=date, distance=float(distance), time=time, note=note)
...
def yearly_total(request):
    ...
    end_date = min(
        ...
        .replace(day=1, month=current_month % 12 + 1)
        .date()
        - timezone.timedelta(days=1),
    )

    current_year_total = (
        Run.objects.filter(date__year=current_year, date__lte=end_date).aggregate(
            total=Sum("distance")
        )["total"]
        or 0
    )
    ...
    previous_year_total = (
        Run.objects.filter(
            date__year=current_year - 1, date__lte=previous_year_end_date
...
def weekly_total(request):
    ...
    total = (
        Run.objects.filter(date__gte=seven_days_ago).aggregate(Sum("distance"))[
            "distance__sum"
        ]
        or 0
    )
...
def yearly_distance_data(request):
    ...
    monthly_data = (
        Run.objects.filter(date__year=current_year)
